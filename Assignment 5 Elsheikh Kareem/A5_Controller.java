...
public class A5_Controller {
    ...
    @FXML
    void saveButtonEventHandler( ActionEvent event ) throws IOException
    {
        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Select an XML File to save");
        fileChooser.setInitialDirectory(new File("."));
        File file = fileChooser.showSaveDialog(btnsave.getScene().getWindow());

        String name = file.getName();
        if(name.matches("[A-Z]\\S{4,}xml$"))
        {
            try (FileOutputStream fos = new FileOutputStream((file)))
            {
                try (XMLEncoder encoder = new XMLEncoder(fos))
                {
                    for (Node child: panelDraw.getChildren())
                    {
                        ...
                        String color = lblRGB.getText();

                        A5Circle circle1 = new A5Circle(x,y,radius,color);

                     encoder.writeObject(circle1);
                     encoder.writeObject(circle1.getX(x));
                     encoder.writeObject(circle1.getY(y));
                     encoder.writeObject(circle1.getRadius(radius));
                     encoder.writeObject(circle1.getColor(color));
        ...
        else System.err.println("File not Valid");
    }
